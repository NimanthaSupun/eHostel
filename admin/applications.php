...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['approve_id'])) {
    ...
    try {
        $stmt = mysqli_prepare($conn, "SELECT user_id FROM applications WHERE application_id = ?");
        ...
        mysqli_stmt_bind_param($alloc, "iii", $app_id, $app['user_id'], $bed_id);
...
$apps = mysqli_query($conn, "SELECT ap.*, u.full_name, u.email, u.reg_no
...
// vacant beds grouped for the allocate dropdown
$vacantBeds = mysqli_query($conn, "SELECT b.bed_id, b.bed_number, r.room_number, r.room_type
                                    ...
                                    WHERE b.status='vacant' ORDER BY r.room_number, b.bed_number");
$bedOptions = [];
...
<div class="card">
    <table>
        ...
        <?php if (mysqli_num_rows($apps) > 0): while ($a = mysqli_fetch_assoc($apps)): ?>
            <tr>
                <td>
                    ...
                    <span style="font-size:0.78rem;color:var(--text-muted);"><?= h($a['email'] ?: 'No Email') ?></span>
                </td>
                <td><?= h($a['reg_no'] ?: '—') ?></td>
                ...
                <td>
                    <?php if ($a['status'] === 'pending'): ?>
                        ...
                        <form method="POST" action="applications.php" style="display:flex;gap:0.4rem;align-items:center;">
                            ...
                            <button type="submit" class="btn btn-sm btn-luxury btn-accent">Approve &amp; Assign</button>
                        </form>
                        <?php else: ?>
        ...
        <?php endwhile; else: ?>
            <tr><td colspan="6" class="empty-state">No student applications submitted yet.</td></tr>
        <?php endif; ?>
        </tbody>
    </table>
</div>
